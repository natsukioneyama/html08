...
(() => {
  ...
  // デスクトップのアイコンから開く（既存の data-type="simple-view" を使用）
  document.addEventListener('click', (e) => {
  ...
  // 背景クリックや Esc で閉じる
  sv.addEventListener('click', (e) => { if (e.target === sv) closeSimple(); });
  document.addEventListener('keydown', (e) => {
    if (sv.classList.contains('open') && e.key === 'Escape') closeSimple();
  });
})();
...
const open = (list, start=0) => {
  ...
  // === Desktop swipe / drag navigation (PC & touch共通) ===
(() => {
  const frame = modal.querySelector('.gm-frame');
  if (!frame) return;
...
// === Mac 二本指スワイプ（wheel deltaX）: 減衰＋ヒステリシス版 ===
(() => {
  const frame = modal.querySelector('.gm-frame');
  if (!frame) return;
...
if (caption) {
  ...
  // 閉じる操作
  ov2.close?.addEventListener('click', () => ov2.hide());
  $('#ov2-modal .ov2-backdrop')?.addEventListener('click', () => ov2.hide());

  v2.closeB?.addEventListener('click', () => v2.hide());
  $('#v2-viewer .v2-backdrop')?.addEventListener('click', () => v2.hide());
  v2.prevB?.addEventListener('click', () => v2.move(-1));
  v2.nextB?.addEventListener('click', () => v2.move(1));

  // キー操作
  window.addEventListener('keydown', e => {
    if ($('#v2-viewer[aria-hidden="false"]')) {
      if (e.key === 'ArrowLeft')  v2.move(-1);
      else if (e.key === 'ArrowRight') v2.move(1);
      else if (e.key === 'Escape') v2.hide();
    } else if ($('#ov2-modal[aria-hidden="false"]')) {
      if (e.key === 'Escape') ov2.hide();
    }
  });

  // タッチスワイプ
  $('#v2-viewer')?.addEventListener('touchstart', e => { v2.touchX = e.touches[0].clientX; }, { passive: true });
  $('#v2-viewer')?.addEventListener('touchend',   e => {
    const dx = e.changedTouches[0].clientX - v2.touchX;
    if (Math.abs(dx) > 40) v2.move(dx < 0 ? 1 : -1);
  }, { passive: true });
})(); // ← ここで終わり（この IIFE だけ残す）

/* ==== Mac 二本指スワイプ：減衰＋ヒステリシス付き ==== */
(() => {
  ...
  frame.addEventListener('wheel', (e) => {
    ...
    if (sign === 0 || s === sign) {
      ...
      sign = s;
      accum = e.deltaX; // 方向切り替え時はリセットして新方向から積み直し
    }
    sign = s;
