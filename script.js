...
// simple-view のうち動画(.mp4)だけ無効化（既存のどのハンドラより先に止める）
document.addEventListener('click', function(e){
  const btn = e.target.closest('.icon[data-type="simple-view"][data-src$=".mp4"]');
  if (!btn) return;
  e.preventDefault();
  e.stopPropagation();
