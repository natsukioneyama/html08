...
(function() {
  ...
  modal.addEventListener('touchend', () => {
    ...
    if (diff > 0) {
      // → スワイプ（前へ）
      document.querySelector('.gm-prev')?.click();
    } else {
      // ← スワイプ（次へ）
      document.querySelector('.gm-next')?.click();
    }
  });
})();
...
// === Mobile: 1st tap shows caption, 2nd tap opens viewer (for #overviewGrid thumbnails) ===
(function () {
  // タッチ判定（デスクトップは従来どおり）
  ...
  // 1回目タップで .tap-armed を付けてキャプション表示、2回目は既存処理へ
  grid.addEventListener('click', (ev) => {
    const item = ev.target.closest('a.jg-entry');
    if (!item) return;

    // .ov-cap が無ければ data-* から生成（data-title / data-line1 / data-line2 を使用）
    ...
    if (!cap) {
      ...
      if (t || l1 || l2) {
        ...
        // a の相対配置が必要なら CSS 側で a{ position:relative; } を指定
        item.style.position = 'relative';
        item.appendChild(cap);
      }
    }

    // キャプションが無ければ2段階タップは不要（そのまま既存クリックへ）
    if (!cap) return;

    // すでに“武装”済み＝2回目タップ → 既存のクリック処理に通す
    if (item.classList.contains('tap-armed')) {
      item.classList.remove('tap-armed');
      return; // ここでは止めない＝既存のギャラリー起動が発火
    }

    // 1回目タップ：キャプションを見せてギャラリー起動を止める
    ev.preventDefault();
    ev.stopPropagation();

    // 他の武装を解除して今回だけ武装
    grid.querySelectorAll('a.jg-entry.tap-armed').forEach(a => { if (a !== item) a.classList.remove('tap-armed'); });
